(require
  hyrule [ -> ->> as-> unless defmain let+ fn+ doto]
  hyrule :readers [%])
...
(defn print-post-test-info [[csv-path None] [html-path None]]
  (let [message (cond
                  (and (is-not None csv-path) (is-not None html-path))
                  (+ f"\n:file_cabinet:  [grey70]Test results saved to "
                     f"{csv-path} and {html-path}.[/grey70]")

                  (or (is-not None csv-path) (is-not None html-path))
                  (+ f"\n:file_cabinet:  [grey70]Test results saved to "
                     f"{(or csv-path html-path)}.[/grey70]")

                  (and (is None csv-path) (is None html-path))
                  (+ f"\n:file_cabinet:  [grey70]Test results "
                     f"[italic] not [/italic] saved!"))]

    (->> message (.print +console+ :justify "center"))))
...
(defn test-summary-df->rich-format [summary-df]
  (let [format-value #%(do f"[bold {%2}]{(str %1)}[/bold {%2}]")]
   (as-> summary-df it
      (.transform-column it :column-name "Pass"
                            :function #%(if (= 0 %1)
                                          (format-value %1 (:fail +colors+))
                                          (format-value %1 (:pass +colors+))))
      (.transform-column it :column-name "Fail"
                            :function #%(if (< 0 %1)
                                          (format-value %1 (:fail +colors+))
                                          (format-value %1 (:pass +colors+)))))))
...
(defn report-test-results [test-results * [csv-path None] [html-path None]]
    (let [df (test-results->df test-results)]
      ...
      (print-post-test-info csv-path html-path)
     ...
     ; (. +console+ (save-svg (/ log-folder (Path f"{test-results-label}.svg")) :theme +rich-theme+)) ; generate image for README
      (unless (is None csv-path)
        (. df (to-csv csv-path :index False)))
      (unless (is None html-path)
